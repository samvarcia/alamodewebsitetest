...
async function updateApprovedSheet(sheets) {
  try {
    // Read data from UNAPPROVED sheet
    const unapprovedRange = 'UNAPPROVED!A2:J';
    const unapprovedResponse = await sheets.spreadsheets.values.get({
      spreadsheetId: process.env.GOOGLE_SHEET_ID,
      range: unapprovedRange,
    });

    const unapprovedRows = unapprovedResponse.data.values;

    if (unapprovedRows && unapprovedRows.length) {
      const approvedRows = unapprovedRows.filter(row => row[8] === 'Y' && row[9] !== 'Y');
      
      if (approvedRows.length) {
        // Append approved rows to APPROVED sheet
        const approvedRange = 'APPROVED!A2:J';
        ...
        // Send approval emails and update emailSent status
        for (let row of approvedRows) {
          const [parties, firstName, , email] = row;
          await sendEmail(
            email,
            `${parties} Party Submission Approved`,
            `Dear ${firstName},\n\nYour submission to our Fashion Week Party has been approved. We look forward to seeing you at the event!\n\nBest regards,\nLOCATION A LA MODE`
          );

          // Update emailSent status in UNAPPROVED sheet
          const rowIndex = unapprovedRows.indexOf(row) + 2; // +2 because sheet is 1-indexed and we have a header row
          await sheets.spreadsheets.values.update({
            spreadsheetId: process.env.GOOGLE_SHEET_ID,
            range: `UNAPPROVED!J${rowIndex}`,
            valueInputOption: 'USER_ENTERED',
            requestBody: {
              values: [['Y']]
            }
    ...
    console.error('Error updating APPROVED sheet:', error);
    throw error;
  }
}
...
export async function GET(request) {
  try {
    ...
    const updatedCount = await updateApprovedSheet(sheets);

    return NextResponse.json({ message: `${updatedCount} entries approved and updated` }, { status: 200 });
  } catch (error) {
    console.error('Error:', error);
    return NextResponse.json({ error: error.message || 'Error checking approvals' }, { status: 500 });
  }
