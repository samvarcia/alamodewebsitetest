...
try {
  const emailResult = await sendInvitateEmail(email, party, pdfBuffer, htmlTemplate);
  let emailStatus;
  if (emailResult.success) {
    emailStatus = 'Sent';
    // Add to approved sheet with timestamp and email status
    ...
    emailStatus = 'Failed';
    // Add to approved sheet with timestamp and failed email status
    await updateApprovedList(sheets, row, attendeeId, nextRow, timestamp, emailStatus);
  ...
  // Add to approved sheet with timestamp and failed email status
  await updateApprovedList(sheets, row, attendeeId, nextRow, timestamp, emailStatus);
  return { success: false, email, message: 'Failed to send invitation', emailStatus: emailStatus, error: error.message };
}
    } else {
    ...
    await sheets.spreadsheets.values.update({
      ...
      requestBody: {
        values: [[
          ...
          'Not Checked In',
          timestamp,
          emailStatus  // Add the email status to the last column
        ]]
      }
    });
